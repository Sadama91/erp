...
@section('page_title', 'Leverancier bewerken: ' . $supplier->name)
...
<div class="max-w-5xl mx-auto p-4">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <h1 class="text-2xl font-bold">Leverancier bewerken: {{ $supplier->name }}</h1>
    </div>
        ...
        <form action="{{ route('suppliers.update', $supplier->id) }}" method="POST">
            ...
            <div class="grid grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="status" id="status" 
                        class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        <option value="1" {{ $supplier->status ? 'selected' : '' }}>Actief</option>
                        <option value="0" {{ !$supplier->status ? 'selected' : '' }}>Inactief</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="telephone" id="telephone" value="{{ old('telephone', $supplier->telephone) }}" 
                        class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    ...
                    <input type="text" name="purchase_via" id="purchase_via" value="{{ old('purchase_via', $supplier->purchase_via) }}" 
                        class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                <div>
                    ...
                    <textarea name="contact_info" id="contact_info" rows="8"
                        class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('contact_info', $supplier->contact_info) }}</textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <input type="text" name="payment_days" id="payment_days" value="{{ old('payment_days', $supplier->payment_days) }}" 
                        class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <label for="terms" class="block font-medium text-gray-700 mt-4">Voorwaarden</label>
                    <textarea name="terms" id="terms" rows="6"
                        class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('terms', $supplier->terms) }}</textarea>
                </div>
                <div>
                    <label for="remarks" class="block font-medium text-gray-700">Opmerkingen</label>
                    <textarea name="remarks" id="remarks" rows="10"
                        class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('remarks', $supplier->remarks) }}</textarea>
                </div>
            </div>
