...
@section('page_title', 'Bestelling bewerken #'. $order->id)

@section('content')
<div class="container mx-auto">
    <form action="{{ route('orders.update', $order->id) }}" method="POST" id="orderForm" onsubmit="return prepareForm()">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-3 gap-4 mb-6 bg-white p-4 rounded-lg shadow w-full">
            ...
            <table class="w-full">
                <tr>
                    ...
                    <td class="w-1/8">
                        <input type="date" name="date" id="date" value="{{ old('date', $order->date) }}" 
                    ...
                    <td class="w-1/8">
                        <input type="text" name="customer_address" id="customer_address" value="{{ old('customer_address', $order->customer_address) }}" 
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-200 focus:border-blue-300 w-48" >
                    </td>
                </tr>
                <tr>
                    ...
                    <td class="w-1/8">
                        <select name="order_source" id="order_source" 
                            ...
                            @foreach ($salesChannels as $channel)
                                <option value="{{ $channel->value }}" {{ old('order_source', $order->order_source) == $channel->value ? 'selected' : '' }}>
                                    {{ $channel->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <td class="w-1/8">
                        <input type="text" name="username" id="username" value="{{ old('username', $order->username) }}" 
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-200 focus:border-blue-300">                    </td>
                    <td class="w-1/8">
                        <label for="postal_code" class="block text-sm font-medium text-gray-700">Postcode</label>
                    </td>
                    <td class="w-1/8">
                        <input type="text" name="postal_code" id="postal_code" value="{{ old('postal_code', $order->postal_code) }}" 
                               class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-200 focus:border-blue-300">
                    </td>
                </tr>
                <tr>
        ...
        <select name="shipping_method" id="shipping_method" 
            ...
            @foreach ($shippingMethods as $method)
                <option value="{{ $method->value }}" 
                    {{ old('shipping_method', $order->shipping_method) == $method->value ? 'selected' : '' }}>
                    {{ $method->name }}
                </option>
            @endforeach
        </select>
    ...
    <td rowspan="2" class="w-1/8">
        <textarea name="notes" id="notes" rows="3" 
            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-200 focus:border-blue-300">{{ old('notes', $order->notes) }}</textarea>
    </td>
    <td class="w-1/8">
        <label for="city" class="block text-sm font-medium text-gray-700">Plaats</label>
    </td>
    <td class="w-1/8">
        <input type="text" name="city" id="city" value="{{ old('city', $order->city) }}" 
            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-200 focus:border-blue-300">
    </td>
</tr>
<tr>
    ...
    <td class="w-1/8">
        <input type="number" name="shipping_cost" id="shipping_cost" value="{{ old('shipping_cost', $order->shipping_cost) }}" 
    ...
    <td class="w-1/8">
        <input type="text" name="country" id="country" value="{{ old('country', $order->country) }}" 
            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-200 focus:border-blue-300">
    </td>
</tr>
                ...
                <tr>
                    ...
                    <td class="w-1/8 py-2">
                        <label for="new_status" class="block text-sm font-medium text-gray-700">Nieuwe status</label>
</td>
<td>
    <select name="new_status" id="new_status"
        ...
        <!-- Geselecteerde producten sectie -->
...
@php
    $selectedProducts = $order->orderItems->map(function ($item) {
        ...
        return [
            ...
            'sku' => $item->product->sku ?? 'Onbekend SKU',
            'quantity' => $item->quantity,
            'sell_price' => $item->original_sales_price,
            'subtotal' => $item->calculated_sell_price ?? 0.00,
            'lastPurchaseUnitPrice' => $item->purchase_price ?? 0.00,
            'LongDescription' => "{$product->sku} - {$product->brand->name} - {$product->name}", // Voeg de lange beschrijving toe
      
        ];
    });
@endphp
<script>
    let selectedProducts = @json($selectedProducts); // Vul de array met geselecteerde producten
...
async function searchProducts() {
    ...
    const selectedProductIds = getSelectedProductIds();  // Haal de geselecteerde product ID's op
    ...
    try {
        ...
        selectedProductIds.forEach(id => url.searchParams.append('selectedProducts[]', id));  // Voeg geselecteerde product ID's toe
        ...
        const filteredProducts = products.map(product => {
            return {
                ...
                LongDescription: `${product.sku} - ${product.brand_name} - ${product.name}`,
...
function displayProducts(products) {
    ...
    productContainer.innerHTML = ''; // Leeg de huidige lijst

    productContainer.classList.remove('hidden'); // Maak de lijst zichtbaar
    ...
    products.forEach(product => {
        ...
        productItem.addEventListener('click', () => {
            selectProduct(product); // Voeg het geselecteerde product toe aan de lijst
        });
...
function selectProduct(product) {
    ...
    updateSelectedProducts(); // Update de geselecteerde productenlijst
    updateTotals(); // Update de totale berekeningen
}

function updateSelectedProducts() {
    const productTableBody = document.getElementById('selected_products');
    
    // Controleer of het element bestaat voordat we de inhoud bijwerken
    if (!productTableBody) {
        console.error("Tabel voor geselecteerde producten niet gevonden!");
        return;
    }
    ...
    selectedProducts.forEach((product, index) => {
        ...
        const sellPrice = parseFloat(product.sell_price) || 0.00;
    ...
    row.innerHTML = `
        ...
        <td>${product.LongDescription || '-'}</td> <!-- Zorg ervoor dat longDescription goed wordt weergegeven -->
        <td>
            <input type="hidden"
        ...
        <td>
            <input type="hidden"
                    ...
                    €${lastPurchaseUnitPrice.toFixed(2)} <!-- Inkoopprijs goed weergeven -->
            <input type="hidden"
        ...
        <td>
            <table><tr><td>
            <a href="#" onclick="resetProduct(${index})" class="text-gray-500 hover:text-gray-700 inline-flex items-center" title="Reset deze rij">
    ...
    feather.replace();  // Zorg ervoor dat de icoontjes correct worden weergegeven
});
