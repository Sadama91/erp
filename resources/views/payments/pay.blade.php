...
<div class="max-w-md mx-auto bg-white p-6 shadow rounded">
    ...
    <form action="{{ route('invoices.storePayment', $invoice) }}" method="POST">
        ...
        <div class="mb-4">
            <label class="block font-semibold">Betaalrekening</label>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="reference" 
                   value="{{ $invoice->id . ' ' . ($invoice->supplier->name ?? '') . ' ' . ($invoice->reference ?? '') }}" 
                   readonly 
...
@section('scripts')
<script>
    // Zorg dat eventuele iconen met feather worden vervangen (als dat nodig is)
    feather.replace();
</script>
@endsection
