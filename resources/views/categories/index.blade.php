...
@section('page_title', 'Overzicht Categorieën')
...
<div class="container mx-auto mt-6">
    ...
    <div class="flex mb-4">
        <a href="{{ route('categories.create') }}" class="bg-blue-600 text-white rounded-lg px-4 py-2 hover:bg-blue-700 transition duration-200">
            Nieuwe Categorie Toevoegen
        </a>
    </div>

    <!-- Filters en zoeken -->
    <form method="GET" action="{{ route('categories.index') }}" class="mb-4 flex justify-between items-center">
        <!-- Zoeken en sorteren -->
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr>
                ...
                <td class="px-4 py-2 whitespace-nowrap">
                    <a href="{{ route('categories.edit', $category->id) }}" class="text-blue-600 hover:text-blue-700 inline-flex items-center transition">
                        <svg class="w-5 h-5 mr-1" data-feather="edit"></svg>
                    </a>
                    <form action="{{ route('categories.destroy', $category->id) }}" method="POST" class="inline" onsubmit="return confirm('Weet je zeker dat je deze categorie wilt verwijderen?')">
                        @csrf
                        @method('DELETE')
