            ...
            <div
                ...
                <div class="bg-white shadow-sm rounded-lg p-4">
                            ...
                            @if (!$category->parent_id)
                                <div class="flex items-center">
                                    <input type="checkbox" id="category_{{ $category->id }}" name="categories[]"
                                        value="{{ $category->id }}" @if (in_array($category->id, $selectedCategory ?? [])) checked @endif
                                        class="mr-2">
                                        ...
                                        @foreach ($category->children as $child)
                                            <div class="flex items-center">
                                                ...
                                                <label for="category_{{ $child->id }}"
                                                    class="text-gray-600">{{ $child->name }}</label>
                                            </div>
                                            @if ($child->children)
                                                <div class="pl-4">
                                                    @foreach ($child->children as $grandchild)
                                                        <div class="flex items-center">
                                                            <input type="checkbox" id="category_{{ $grandchild->id }}"
                                                                ...
                                                                @if (in_array($grandchild->id, $selectedCategory ?? [])) checked @endif
                                                                class="mr-2">
                    ...
                    <div class="bg-white shadow-sm rounded-lg p-4 w-full">
                        ...
                        <div>
                            <label for="product_type_id" class="block font-medium text-gray-700">Producttype</label>
                            <select id="product_type_id" name="product_type_id" class="w-full border-gray-300 rounded-md">
                                @foreach ($productTypes as $productType)
                                    <option value="{{ $productType->id }}"
                                        {{ old('product_type_id', $product->productType) == $productType->value ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="sales_chanel[]" name="sales_chanel[]" class="w-full border-gray-300 rounded-md"
                                multiple>
                                @foreach ($salesChanels as $salesChanel)
                                    <option value="{{ $salesChanel->id }}"
                                        {{ old('sales_chanel', $product->sales_chanel) == $salesChanel->value ? 'selected' : '' }}>
                            ...
                            <select id="shipping_class" name="shipping_class" class="w-full border-gray-300 rounded-md">
                                @foreach ($shippingClasses as $shippingClass)
                                    <option value="{{ $shippingClass->id }}"
                                        {{ old('shipping_class', $product->shipping_class) == $shippingClass->id ? 'selected' : '' }}>
