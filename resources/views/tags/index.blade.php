...
@section('page_title', 'Overzicht Tags')
...
<div class="container mx-auto mt-6">
    ...
    <!-- Nieuwe tag aanmaken -->
    <div class="flex mb-4">
        <a href="{{ route('tags.create') }}" class="bg-blue-600 text-white rounded-lg px-4 py-2 hover:bg-blue-700 transition duration-200">
            Nieuwe Tag Toevoegen
        </a>
    </div>

    <!-- Filters en zoeken -->
    <form method="GET" action="{{ route('tags.index') }}" class="mb-4 flex justify-between items-center">
        <!-- Zoeken en sorteren -->
    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @foreach ($tags as $tag)
            <tr>
                <td class="px-4 py-2 whitespace-nowrap">{{ $tag->name }}</td>
                <td class="px-4 py-2 whitespace-nowrap">{{ $tag->slug }}</td>
                <td class="px-4 py-2 whitespace-nowrap">
                    <a href="{{ route('tags.edit', $tag->id) }}" class="text-blue-600 hover:text-blue-700 inline-flex items-center transition">
                        <svg class="w-5 h-5 mr-1" data-feather="edit"></svg>
                    </a>
                    <form action="{{ route('tags.destroy', $tag->id) }}" method="POST" class="inline" onsubmit="return confirm('Weet je zeker dat je deze tag wilt verwijderen?')">
                        @csrf
                        @method('DELETE')
